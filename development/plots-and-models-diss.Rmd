...
Quick example of non-aligned, then sig-aligned.  

```{r}
sig1 <- blue_aligned %>% filter(unique_id == "BL B-6", operator == "Anyesha", 
                        machine == "Sneox1", round == "Round 1",
                        bullet == "Bullet 1")
sig2 <- blue_aligned %>% filter(unique_id == "BL B-6", operator == "Anyesha", 
                                machine == "Sneox1", round == "Round 1", 
                                bullet == "Bullet 3")

sigs_nonalign <- ggplot() + 
  geom_line(aes(x = x, y = sig), data = sig1$sigs[[1]], color = "cadetblue3") + 
  geom_line(aes(x = x, y = sig), data = sig2$sigs[[1]], color = "midnightblue") + 
  theme_bw() + 
  labs(x = expression(paste("Relative Location x"[i], " (", mu, "m)")),
       y = expression(paste("Signature Height z"[i], " (", mu, "m)")), 
       title = "Two known-match signatures, raw extracted signatures")

sigs_align <- ggplot() + 
  geom_line(aes(x = x_aligned, y = sig1), data = sig2$sigs_aligned[[1]], color = "cadetblue3") + 
  geom_line(aes(x = x_aligned, y = sig2), data = sig2$sigs_aligned[[1]], color = "midnightblue") + 
  theme_bw() + 
  labs(x = expression(paste("Relative Location x"[i], " (", mu, "m)")),
       y = expression(paste("Signature Height z"[i], " (", mu, "m)")), 
       title = "Two known-match signatures, coaligned in the x direction")

grid.arrange(sigs_nonalign, sigs_align, nrow = 2)
```
...
blue_df %>%
  ...
  #scale_color_manual(name = "Phase", values = zissou_palette10) + 
  geom_point(aes(x = factor(unique_id), y = sigma_mean), pch = 23, color = "black", fill = "white") + 
  facet_wrap(~grp,  nrow = 2) + 
  theme_bw() + 
...
Signature models without tank rash lands... 

```{r}
tank_rash_gt <- read_csv("../data/variability_scans/tank_rash_gt.csv")
all_aligned_long <- left_join(all_aligned_long, tank_rash_gt)

# we need to do new models for: 
# orange-1
orange_l1_long_tr <- all_aligned_long %>% 
  filter(unique_id == "BL O-1", keep_x == T, tank_rash == F) %>%
  mutate(fixed_BLi = paste0(unique_id, "-", x_aligned))

orange_l1_df_tr <- data.frame(window_size = 100) %>% ## 3 singular
  mutate(sampling_ests = purrr::map(.x = window_size, .f = function(x){
    subsample_sigs(sig_aligned_data = orange_l1_long_tr, n_phases = 10, window_size = x)
  })) %>% 
  unnest(cols = c(sampling_ests)) %>% 
  unnest(cols = c(sigmas))

# orange-4
orange_l4_long_tr <- all_aligned_long %>% 
  filter(unique_id == "BL O-4", keep_x == T, tank_rash == F) %>%
  mutate(fixed_BLi = paste0(unique_id, "-", x_aligned))

orange_l4_df_tr <- data.frame(window_size = 100) %>% ## 4 singular
  mutate(sampling_ests = purrr::map(.x = window_size, .f = function(x){
    subsample_sigs(sig_aligned_data = orange_l4_long_tr, n_phases = 10, window_size = x)
  })) %>% 
  unnest(cols = c(sampling_ests)) %>% 
  unnest(cols = c(sigmas))

# orange-6
orange_l6_long_tr <- all_aligned_long %>% 
  filter(unique_id == "BL O-6", keep_x == T, tank_rash == F) %>%
  mutate(fixed_BLi = paste0(unique_id, "-", x_aligned))

orange_l6_df_tr <- data.frame(window_size = 100) %>% ## bunch of issues - only 1 bullet...
  mutate(sampling_ests = purrr::map(.x = window_size, .f = function(x){
    subsample_sigs(sig_aligned_data = orange_l6_long_tr, n_phases = 10, window_size = x)
  })) %>% 
  unnest(cols = c(sampling_ests)) %>% 
  unnest(cols = c(sigmas))
...
orange_pooled_long_tr <- all_aligned_long %>%  
  filter(barrel == "Barrel Orange", keep_x == T, tank_rash == F) %>%
  mutate(fixed_BLi = paste0(unique_id, "-", x_aligned))

orange_pooled_df_tr <- data.frame(window_size = 100) %>% ### 8 singular, 1 failed convergence
  mutate(sampling_ests = purrr::map(.x = window_size, .f = function(x){
    subsample_sigs_pooled(sig_aligned_data = orange_pooled_long_tr, n_phases = 10, window_size = x)
  })) %>% 
  unnest(cols = c(sampling_ests)) %>% 
  unnest(cols = c(sigmas))


orange_df_tr <- rbind(orange_l1_df_tr, orange_l4_df_tr, orange_l6_df_tr, orange_pooled_df_tr)
orange_df_tr <- orange_df_tr %>%
  mutate(grp = factor(grp, levels = c("bullet:fixed_BLi", "operator:fixed_BLi", 
                                      "machine:fixed_BLi", "bullet:operator:fixed_BLi", 
                                      "bullet:machine:fixed_BLi", "operator:machine:fixed_BLi", 
                                      "operator:bullet:machine:fixed_BLi", "Residual"), 
                      labels = c("Bullet", "Operator", "Machine", 
                                 "Bullet-Operator", "Bullet-Machine", "Operator-Machine", 
                                 "Operator-Bullet-Machine", "Residual")))
orange_df <- read_csv("/Volumes/databuddy/Variability/data/orange_signature_model_results.csv")
orange_df$tr_model <- F
orange_df_tr$tr_model <- T

orange_df_2 <- rbind(orange_df, orange_df_tr)

orange_df_2 <- orange_df_2 %>%
  mutate(unique_id = factor(unique_id, 
                            levels = c("BL O-1", "BL O-2", "BL O-3", 
                                       "BL O-4", "BL O-5", "BL O-6", "Pooled"), 
                            labels = c("O-1", "O-2", "O-3", 
                                       "O-4", "O-5", "O-6", "Pooled"))) %>%
...
orange_df_2 %>%
  ...
  mutate(sigma_mean = mean(sdcor)) %>%
  ungroup() %>%
  #filter(unique_id %in% c("O-1", "O-4", "O-6", "Pooled")) %>%
  ggplot() + 
  geom_boxplot(aes(x = factor(unique_id), y = sdcor, 
                   fill = factor(tr_model), color = factor(tr_model)), 
               alpha = 0.7, position = "identity") + 
  scale_fill_manual(name = "Tank Rash Filtered", values = c(NA, "darkorange")) + 
  scale_color_manual(name = "Tank Rash Filtered", values = c("grey60", "black")) + 
  geom_vline(xintercept = c("O-1", "O-4", "O-6", "Pooled"), lty = 2, color = "grey70") + 
  #geom_point(aes(x = factor(unique_id), y = sdcor, color = factor(phase_id)), alpha = 0.6) + 
  #scale_color_manual(name = "Phase", values = zissou_palette10) + 
...
# pink-6
pink_l6_long_tr <- all_aligned_long %>% 
  filter(unique_id == "BL P-6", keep_x == T, tank_rash == F) %>%
  mutate(fixed_BLi = paste0(unique_id, "-", x_aligned))

pink_l6_df_tr <- data.frame(window_size = 100) %>% ## 3 singular
  mutate(sampling_ests = purrr::map(.x = window_size, .f = function(x){
    subsample_sigs(sig_aligned_data = pink_l6_long_tr, n_phases = 10, window_size = x)
  })) %>% 
  unnest(cols = c(sampling_ests)) %>% 
  unnest(cols = c(sigmas))
...
pink_pooled_long_tr <- all_aligned_long %>%  
  filter(barrel == "Barrel Pink", keep_x == T, tank_rash == F) %>%
  mutate(fixed_BLi = paste0(unique_id, "-", x_aligned))

pink_pooled_df_tr <- data.frame(window_size = 100) %>% ### 8 singular, 1 failed convergence
  mutate(sampling_ests = purrr::map(.x = window_size, .f = function(x){
    subsample_sigs_pooled(sig_aligned_data = pink_pooled_long_tr, n_phases = 10, window_size = x)
  })) %>% 
  unnest(cols = c(sampling_ests)) %>% 
  unnest(cols = c(sigmas))


pink_df_tr <- rbind(pink_l6_df_tr, pink_pooled_df_tr)
pink_df_tr <- pink_df_tr %>%
  mutate(grp = factor(grp, levels = c("bullet:fixed_BLi", "operator:fixed_BLi", 
                                      "machine:fixed_BLi", "bullet:operator:fixed_BLi", 
                                      "bullet:machine:fixed_BLi", "operator:machine:fixed_BLi", 
                                      "operator:bullet:machine:fixed_BLi", "Residual"), 
                      labels = c("Bullet", "Operator", "Machine", 
                                 "Bullet-Operator", "Bullet-Machine", "Operator-Machine", 
                                 "Operator-Bullet-Machine", "Residual")))
pink_df <- read_csv("/Volumes/databuddy/Variability/data/pink_signature_model_results.csv")
pink_df$tr_model <- F
pink_df_tr$tr_model <- T

pink_df_2 <- rbind(pink_df, pink_df_tr)

pink_df_2 <- pink_df_2 %>%
  mutate(unique_id = factor(unique_id, 
                            levels = c("BL P-1", "BL P-2", "BL P-3", 
                                       "BL P-4", "BL P-5", "BL P-6", "Pooled"), 
...
pink_df_2 %>%
  ...
  geom_boxplot(aes(x = factor(unique_id), y = sdcor, 
                   fill = factor(tr_model), color = factor(tr_model)), 
               alpha = 0.7, position = "identity") + 
  scale_fill_manual(name = "Tank Rash Filtered", values = c(NA, "salmon")) + 
  scale_color_manual(name = "Tank Rash Filtered", values = c("grey60", "black")) + 
  geom_vline(xintercept = c("P-6", "Pooled"), lty = 2, color = "grey70") + 
  #geom_point(aes(x = factor(unique_id), y = sdcor, color = factor(phase_id)), alpha = 0.6) + 
  #scale_color_manual(name = "Phase", values = zissou_palette10) + 
  geom_point(aes(x = factor(unique_id), y = sigma_mean, color = factor(tr_model)), pch = 23, fill = "white") +
  facet_wrap(~grp,  nrow = 2) + 
...
model_sigmas_all_2 %>%
  ...
  facet_grid(barrel~tank_rash) + 
  scale_color_manual(name = "Pairing IDs Used", values = c("goldenrod", "grey60", "purple4")) + 
  scale_shape_manual(name = "Pairing IDs Used", values = c(8, 17, 15)) + 
  theme_bw() + 
  labs(x = "Effect type", y = expression(paste("Estimated sigma value (in ", mu, "m)")), 
       title = "Pairwise model estimates by barrel and model",
       subtitle = "with and without tank rash lands included") + 
  theme(plot.subtitle = element_text(face = "italic"))
...
model_pairwise_blue_sub <- lmer(rfscore~unique_id_pairing_fixed + 
                              ...
                              (1|operator_pairing_fixed:machine_pairing_fixed:unique_id_pairing_fixed) + 
                              (1|bullet_pairing_fixed:operator_pairing_fixed:machine_pairing_fixed:unique_id_pairing_fixed), 
                            data = blue_pairwise %>% sample_frac(0.1))
...
model_pairwise_blue_ss_sub <- lmer(rfscore~unique_id_pairing_fixed + 
                              ...
                              (1|operator_pairing_fixed:machine_pairing_fixed:unique_id_pairing_fixed) + 
                              (1|bullet_pairing_fixed:operator_pairing_fixed:machine_pairing_fixed:unique_id_pairing_fixed), 
                            data = blue_pairwise %>% filter(ground_truth == "Same Source") %>% sample_frac(0.1))
...
model_pairwise_blue_ds_sub <- lmer(rfscore~unique_id_pairing_fixed + 
                              ...
                              (1|operator_pairing_fixed:machine_pairing_fixed:unique_id_pairing_fixed) + 
                              (1|bullet_pairing_fixed:operator_pairing_fixed:machine_pairing_fixed:unique_id_pairing_fixed), 
                            data = blue_pairwise %>% filter(ground_truth == "Different Source") %>% sample_frac(0.1))

model_sigmas_blue_sub <- as.data.frame(VarCorr(model_pairwise_blue_sub))[,c("grp", "sdcor")]
model_sigmas_blue_ss_sub <- as.data.frame(VarCorr(model_pairwise_blue_ss_sub))[,c("grp", "sdcor")]
model_sigmas_blue_ds_sub <- as.data.frame(VarCorr(model_pairwise_blue_ds_sub))[,c("grp", "sdcor")]
model_sigmas_blue_sub <- model_sigmas_blue_sub %>% mutate(model = "All", barrel = "Blue")
model_sigmas_blue_ss_sub <- model_sigmas_blue_ss_sub %>% mutate(model = "Same Source", barrel = "Blue")
model_sigmas_blue_ds_sub <- model_sigmas_blue_ds_sub %>% mutate(model = "Different Source", barrel = "Blue")

blue_ss_boot_ci_sub <- confint(model_pairwise_blue_ss_sub, parm = "theta_", level = 0.95, method = "boot")
blue_ds_boot_ci_sub <- confint(model_pairwise_blue_ds_sub, parm = "theta_", level = 0.95, method = "boot")
blue_boot_ci_sub <- confint(model_pairwise_blue_sub, parm = "theta_", level = 0.95, method = "boot")
# 
...
o6_bullets <- all_aligned %>% 
  filter(unique_id == "BL O-6", operator == "Connor", round == "Round 1", machine == "Sneox1") %>% pull(source)

o4_bullets <- all_aligned %>% 
  filter(unique_id == "BL O-4", operator == "Connor", round == "Round 1", machine == "Sneox1") %>% pull(source)

o1_bullets <- all_aligned %>% 
  filter(unique_id == "BL O-1", operator == "Connor", round == "Round 1", machine == "Sneox1") %>% pull(source)
...
p6_bullets <- all_aligned %>% 
  filter(unique_id == "BL P-6", operator == "Connor", round == "Round 1", machine == "Sneox1") %>% pull(source)

p2_bullets <- all_aligned %>% 
  filter(unique_id == "BL P-2", operator == "Connor", round == "Round 1", machine == "Sneox1") %>% pull(source)
...
x3ptools::image_x3p(tank_rash_x3ps$x3p[[9]])
...
x3p <- tank_rash_x3ps$x3p[[2]]
filename <- "../images/tank-rash-example1.png"
x3ptools::image_x3p(x3p, file = filename, size = c(3500, 1500), zoom = 0.5)

x3p <- tank_rash_x3ps$x3p[[1]]
filename <- "../images/tank-rash-comparison1.png"
x3ptools::image_x3p(x3p, file = filename, size = c(3500, 1500), zoom = 0.5)

x3p <- tank_rash_x3ps$x3p[[12]]
filename <- "../images/tank-rash-example2.png"
x3ptools::image_x3p(x3p, file = filename, size = c(3500, 1500), zoom = 0.5)

x3p <- tank_rash_x3ps$x3p[[11]]
filename <- "../images/tank-rash-comparison2.png"
x3ptools::image_x3p(x3p, file = filename, size = c(3500, 1500), zoom = 0.5)
