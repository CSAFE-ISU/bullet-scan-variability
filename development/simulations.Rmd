...
## INTERACTIONS WITH LOCATION MODEL SIMULATIONS
```{r}
numCores <- 12 # on the server
registerDoParallel(cores = numCores)
...
sigma_operator = .08
sigma_machine = .1
sigma_bullet_operator = .2
sigma_bullet_machine = .1
sigma_operator_machine = .001
sigma_operator_bullet_machine = .0001
sigma_error = 0.5
n_sims <- 1:1000
reml_val <- c(T, F)
conv_val <- c("bobyqa", "Nelder_Mead")
...
sim_wrapper_df <- expand.grid(n_bullets = n_bullets, n_operators = n_operators, n_machines = n_machines,
                              ...
                              sigma_operator_bullet_machine = sigma_operator_bullet_machine, 
                              sigma_error = sigma_error, 
                              reml_val = reml_val, conv_val = conv_val,
                              n_sims = n_sims)
...
list_out = foreach(m = 1:nrow(sim_wrapper_df)) %dopar% {
  ...
  sim_data <- sim_data_skeleton %>% 
    nest(data = -c(bullet,location_names)) %>%
    mutate(bullet_eff = rnorm(n_bullets*n_locations, sd = sigma_bullet)) %>%
    unnest(cols = c(data)) %>%
    nest(data = -c(operator,location_names)) %>%
    mutate(operator_eff = rnorm(n_operators*n_locations, sd = sigma_operator)) %>%
    unnest(cols = c(data)) %>%
    nest(data = -c(machine,location_names)) %>%
    mutate(machine_eff = rnorm(n_machines*n_locations, sd = sigma_machine)) %>%
    unnest(cols = c(data)) %>%
    nest(data = -c(bullet, operator, location_names)) %>%
    mutate(bullet_operator_eff = rnorm(n_bullets*n_operators*n_locations, sd = sigma_bullet_operator)) %>%
    unnest(cols = c(data)) %>%
    nest(data = -c(bullet, machine, location_names)) %>%
    mutate(bullet_machine_eff = rnorm(n_bullets*n_machines*n_locations, sd = sigma_bullet_machine)) %>%
    unnest(cols = c(data)) %>%
    nest(data = -c(operator, machine, location_names)) %>%
    mutate(operator_machine_eff = rnorm(n_operators*n_machines*n_locations, sd = sigma_operator_machine)) %>%
    unnest(cols = c(data)) %>%
    nest(data = -c(bullet, operator, machine, location_names)) %>%
    mutate(operator_bullet_machine_eff = rnorm(n_bullets*n_operators*n_machines*n_locations, sd = sigma_operator_bullet_machine)) %>%
    unnest(cols = c(data)) %>%
    mutate(error_eff = rnorm(nrow(sim_data_skeleton), sd = sigma_error))
  ...
  sim_data <- sim_data %>% mutate(response = location_mus + bullet_eff + operator_eff + machine_eff + 
                      bullet_operator_eff + bullet_machine_eff + operator_machine_eff + 
                        operator_bullet_machine_eff + 
                        error_eff)


  sim_model_fit <- lmer(response~location_names + (1|bullet:location_names) + (1|operator:location_names) + (1|machine:location_names) + 
                        (1|bullet:operator:location_names) + (1|bullet:machine:location_names) + (1|operator:machine:location_names) + 
                          (1|operator:bullet:machine:location_names), 
                      data = sim_data, 
                      REML = data$reml_val, 
                      control = lmerControl(check.conv.grad = .makeCC("warning", tol = 0.1), 
                                             check.conv.singular = .makeCC("message", tol = 0.0000001),
                                            optimizer = as.character(data$conv_val)))
...
saveRDS(sim_wrapper_df, "../data/simulation/simulation_orange_locations_sim.rda")
...
sim_wrapper_df %>% 
  ...
  filter(singular == T) %>%
  ggplot() + 
  geom_density(aes(x = sdcor, y = ..scaled..), color = "white", alpha = 0.5, fill = "grey50") + 
  geom_vline(aes(xintercept = true_val)) + 
...
saveRDS(sim_wrapper_df, "../data/simulation/simulation_pink_locations_sim.rda")
...
sigma_operator = .03
sigma_machine = .04
sigma_bullet_operator = .3
sigma_bullet_machine = .001
sigma_operator_machine = .005
sigma_operator_bullet_machine = .001
sigma_error = 0.5
n_sims <- 1:1000
reml_val <- c(T, F)
conv_val <- c("bobyqa", "Nelder_Mead")
...
saveRDS(sim_wrapper_df, "../data/simulation/simulation_green_locations_sim.rda")
...
sigma_bullet_operator = .2
sigma_bullet_machine = .1
sigma_operator_machine = .04
sigma_operator_bullet_machine = .05
sigma_error = 0.5
n_sims <- 1:1000
reml_val <- c(T, F)
conv_val <- c("bobyqa", "Nelder_Mead")
...
saveRDS(sim_wrapper_df, "../data/simulation/simulation_orange_locations_sim_updated_effects.rda")
...
sigma_bullet_operator = .2
sigma_bullet_machine = .2
sigma_operator_machine = .15
sigma_operator_bullet_machine = .15
sigma_error = 1.0
n_sims <- 1:1000
reml_val <- c(T, F)
conv_val <- c("bobyqa", "Nelder_Mead")
...
saveRDS(sim_wrapper_df, "../data/simulation/simulation_pink_locations_sim_updated.rda")
