...
Now, we want to grab recapture data from only Round 4, and save those as their own file.  
```{r, echo = F, warning = F, message = F}
user_recaptures <- user_meas %>% filter(Round == "Round 4") %>% select(-ccdata)
#saveRDS(user_recaptures, "../data/user_recaptures.rda")
```
...
Remove recapture data so we only have "Scan 1" from each round.  
```{r, echo = F, warning = F, message = F}
user_crosscuts <- user_crosscuts %>% filter(Scan == "Scan 1")
```
