...
orange_sigs <- orange_sigs %>% filter(source %notin% scan_filters)
orange_sigs <- left_join(orange_sigs, scan_renames)
orange_sigs <- orange_sigs %>% mutate(barrel = ifelse(is.na(barrel_updated), barrel, barrel_updated), 
...
orange_sigs <- orange_sigs %>% 
  mutate(scan_id = paste0(barrel, "-", land, "-", bullet, "-", 
                          operator, "-", machine, "-", round))
orange_sigs <- orange_sigs %>% select(-c(barrel_updated, bullet_updated, machine_updated, crosscut_safe))
...
pink_sigs <- pink_sigs %>% filter(source %notin% scan_filters)
pink_sigs <- left_join(pink_sigs, scan_renames)
pink_sigs <- pink_sigs %>% mutate(barrel = ifelse(is.na(barrel_updated), barrel, barrel_updated), 
                                      bullet = ifelse(is.na(bullet_updated), bullet, bullet_updated), 
                                      machine = ifelse(is.na(machine_updated), machine, machine_updated))
pink_sigs <- pink_sigs %>% 
  mutate(scan_id = paste0(barrel, "-", land, "-", bullet, "-", 
                          operator, "-", machine, "-", round))
pink_sigs <- pink_sigs %>% select(-c(barrel_updated, bullet_updated, machine_updated, crosscut_safe))
...
blue_all <- rbind(blue_r1, blue_r2, blue_r3, blue_r4, blue_r5)
...
blue_sigs <- blue_sigs %>% filter(source %notin% scan_filters)
blue_sigs <- left_join(blue_sigs, scan_renames)
blue_sigs <- blue_sigs %>% mutate(barrel = ifelse(is.na(barrel_updated), barrel, barrel_updated), 
                                      bullet = ifelse(is.na(bullet_updated), bullet, bullet_updated), 
                                      machine = ifelse(is.na(machine_updated), machine, machine_updated))
blue_sigs <- blue_sigs %>% 
  mutate(scan_id = paste0(barrel, "-", land, "-", bullet, "-", 
                          operator, "-", machine, "-", round))
blue_sigs <- blue_sigs %>% select(-c(barrel_updated, bullet_updated, machine_updated, crosscut_safe))
